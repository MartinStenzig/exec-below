    ...
    const filePromise = new Promise((resolve, reject) => {
      ...
      if (fileSystem.lstatSync(fileLocation).isDirectory() && !_isHiddenUnixPath(aFile) && aDepth < _optionProcessingDepth) {
        new Promise((resolve, reject) => {
          ...
          // console.log('Location: ', fileLocation, 'Command: ', command, params)
...
function _determineCommand (aPath, aDirectoryName) {
  return _commandToExecute.replace(REPLACE_PATTERN_PATH, aPath).replace(REPLACE_PATTERN_DIRECTORY, aDirectoryName).replace(REPLACE_PATTERN_STARTING_PATH, _optionStartingPath)
}
...
function _determineParameterArray (aPath, aDirectoryName) {
  ...
  for (let z = 0; z < _commandParameters.length; z++) {
    returnArray.push(_commandParameters[z].replace(REPLACE_PATTERN_PATH, aPath).replace(REPLACE_PATTERN_DIRECTORY, aDirectoryName).replace(REPLACE_PATTERN_STARTING_PATH, _optionStartingPath))
  }
  return returnArray
}
